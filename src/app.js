(() => {
  ...
  var INITIAL_STOCK = [
    ...
    },
    {
      ...
      minThreshold: 3,
      offcuts: [
        { id: "oc-3", length: 18, quantity: 3, location: "Shelf B-2", jobRef: "Fixture Plates", dateCreated: "2026-08-18" }
      ],
    ...
    },
    {
      ...
      minThreshold: 3,
      offcuts: [
        { id: "oc-4", length: 22, quantity: 1, location: "Drop Bin 3", jobRef: "JOB-398", dateCreated: "2026-08-15" }
      ],
    ...
    },
    {
      ...
      minThreshold: 2,
      offcuts: [
        { id: "oc-6", length: 16, quantity: 1, location: "Stainless Tube Rack", jobRef: "Food Grade Pins", dateCreated: "2026-08-23" }
      ],
    ...
    },
    {
      ...
      minThreshold: 2,
      offcuts: [
        { id: "oc-7", length: 9, quantity: 2, location: "Brass Bin B-1", jobRef: "Fitting Hex Plugs", dateCreated: "2026-08-17" }
      ],
    ...
    },
    {
      ...
      minThreshold: 2,
      offcuts: [
        { id: "oc-8", length: 12, quantity: 4, location: "Plastic Drawer P-1", jobRef: "Insulator Bushings", dateCreated: "2026-08-14" }
      ],
  ...
  var INITIAL_TOOLS = [
    ...
    },
    {
      ...
      name: '1/2" 3-Flute High-Helix Aluminum End Mill 45\xB0',
      category: "end-mill",
      toolType: "Aluminum Rougher/Finisher",
      sizeDiameter: '0.500"',
      flutes: "3-Flute 45\xB0 Helix",
    ...
    },
    {
      ...
      name: '2.00" 45\xB0 5-Insert Indexable Face Mill',
      category: "face-mill",
    ...
    },
    {
      ...
      toolType: "Turning Insert (80\xB0 Trigon)",
    ...
    },
    {
      ...
      name: '1/16" - 1/2" by 64ths Heavy Duty 135\xB0 Split Point M42 Cobalt Drill Index',
      category: "drill-bit",
      toolType: "Jobber Drill Bit Set",
      sizeDiameter: '1/16" to 1/2" (29 sizes)',
      flutes: "2-Flute 135\xB0 Split Point",
  ...
  // src/storage.ts
  var STOCK_STORAGE_KEY = "machine_shop_stock_data_v2";
  var TOOLS_STORAGE_KEY = "machine_shop_tools_data_v2";
  var PARTS_STORAGE_KEY = "machine_shop_parts_data_v2";
  var CUTS_STORAGE_KEY = "machine_shop_cuts_log_v2";
  function loadStock() {
    try {
  ...
  function saveMachineParts(parts) {
    try {
  ...
  function exportStockToCSV(items) {
    const headers = [
      "SKU",
      "Name",
      "Category",
  ...
  function exportToolsToCSV(tools) {
    const headers = [
      "SKU",
      "Tool Name",
      "Category",
  ...
  function exportPartsToCSV(parts) {
    const headers = [
      "Part Number / OEM",
      "Part Description",
      "Machine / Asset",
  ...
  function formatDimensions(item) {
    ...
    if (item.shape === "round-bar" || item.shape === "drill-rod") {
      return `\xD8${item.diameter || 0}" \xD7 ${item.length || 0}"`;
    }
    if (item.shape === "hex-bar") {
      return `${item.diameter || 0}" HEX \xD7 ${item.length || 0}"`;
    }
    if (item.shape === "flat-bar") {
      return `${item.thickness || 0}" \xD7 ${item.width || 0}" \xD7 ${item.length || 0}"`;
    }
    if (item.shape === "square-bar") {
      return `${item.width || item.thickness || 0}" SQ \xD7 ${item.length || 0}"`;
    }
    if (item.shape === "round-tube") {
      return `OD \xD8${item.diameter || 0}" \xD7 ${item.wallThickness || 0}" Wall \xD7 ${item.length || 0}"`;
    }
    if (item.shape === "square-tube") {
      return `${item.width || 0}" SQ Tube \xD7 ${item.wallThickness || 0}" Wall \xD7 ${item.length || 0}"`;
    }
    if (item.shape === "plate-sheet") {
      return `${item.thickness || 0}" Thick \xD7 ${item.width || 0}" W \xD7 ${item.length || 0}" L`;
    }
    if (item.shape === "angle-iron") {
      return `${item.width || 0}" \xD7 ${item.thickness || 0}" Angle \xD7 ${item.length || 0}"`;
    }
  ...
  function getPartCategoryBadge(category) {
    switch (category) {
      case "belts-pulleys":
  ...
  function confirmDelete(message, title = "Confirm Delete") {
    const modal = document.getElementById("modal-confirm-delete");
  ...
  // src/modules/holdToTick.ts
  function attachHoldToTick(element, options) {
    const initialDelay = options.initialDelayMs ?? 1e3;
  ...
  function showToast(message, type = "success") {
    ...
    const borderCol = type === "danger" ? "border-rose-600 bg-rose-950/90 text-rose-200" : type === "warning" ? "border-amber-600 bg-amber-950/90 text-amber-200" : type === "info" ? "border-blue-600 bg-blue-950/90 text-blue-200" : "border-emerald-600 bg-[#12241a]/95 text-emerald-200";
  ...
  // src/modules/materialsView.ts
  function renderMaterialsTable(items, selectedIds2, onQtyChange) {
    if (items.length === 0) return "";
    return items.map((item) => {
      const isSelected = selectedIds2.has(item.id);
      ...
      return `
        <tr class="hover:bg-[#181c23] transition-colors ${isSelected ? "bg-blue-950/20" : ""}" data-item-id="${item.id}">
          ...
          <!-- Offcuts Drops -->
          <td class="py-2.5 px-3 text-center">
            ${totalOffcuts > 0 ? `<button type="button" data-action="manage-offcuts" data-id="${item.id}" class="inline-flex items-center gap-1 px-2 py-0.5 rounded text-[11px] font-mono bg-amber-950/50 text-amber-300 border border-amber-700/50 hover:bg-amber-900/60">
                    <span>${totalOffcuts} drops</span>
                    <span class="text-[9px] opacity-75">\u25B6</span>
                  </button>` : `<button type="button" data-action="manage-offcuts" data-id="${item.id}" class="text-[11px] font-mono text-[#5a6578] hover:text-[#cbd5e1] px-1.5 py-0.5 border border-dashed border-[#2d323b] rounded">
                    + drop
                  </button>`}
          </td>
  ...
  function renderMaterialsCards(items, selectedIds2) {
    return items.map((item) => {
      const isSelected = selectedIds2.has(item.id);
      const catBadge = getCategoryBadge(item.category);
      const statusBadge = getStatusBadge(item.status);
  ...
  // src/modules/toolsView.ts
  function renderToolsTable(tools, selectedIds2) {
    if (tools.length === 0) return "";
    return tools.map((tool) => {
      const isSelected = selectedIds2.has(tool.id);
  ...
  function renderToolsCards(tools, selectedIds2) {
    return tools.map((tool) => {
      const isSelected = selectedIds2.has(tool.id);
      const catBadge = getToolCategoryBadge(tool.category);
      const statusBadge = getStatusBadge(tool.condition);
  ...
  // src/modules/partsView.ts
  function renderPartsTable(parts, selectedIds2) {
    if (parts.length === 0) return "";
    return parts.map((part) => {
      const isSelected = selectedIds2.has(part.id);
  ...
  function renderPartsCards(parts, selectedIds2) {
    return parts.map((part) => {
      const isSelected = selectedIds2.has(part.id);
      const catBadge = getPartCategoryBadge(part.category);
      const statusBadge = getStatusBadge(part.criticality);
      ...
      return `
        <div class="metal-card rounded p-3 space-y-2.5 ${isSelected ? "border-blue-500" : ""}" data-item-id="${part.id}">
          ...
          <div class="flex items-center justify-between pt-1 border-t border-[#232832]">
            ...
            <!-- Card Actions -->
            <div class="flex items-center gap-1">
              <button type="button" data-action="qr-part" data-id="${part.id}" class="btn-metal p-1 rounded text-amber-300" title="QR Asset Tag">
                <svg class="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><rect width="5" height="5" x="3" y="3" rx="1"/><rect width="5" height="5" x="16" y="3" rx="1"/><rect width="5" height="5" x="3" y="16" rx="1"/><path d="M21 16h-3a2 2 0 0 0-2 2v3"/><path d="M21 21v.01"/><path d="M12 7v3a2 2 0 0 1-2 2H7"/></svg>
              </button>
  ...
  // src/modules/auditView.ts
  function renderAuditTable(stock, tools, parts) {
    const allEntries = [];
    stock.forEach((s) => {
      allEntries.push({
        id: s.id,
    ...
    return allEntries.map((entry) => {
      ...
      return `
        <tr class="hover:bg-[#181c23] transition-colors" data-audit-id="${entry.id}" data-audit-domain="${entry.domain}">
          ...
          <td class="py-2.5 px-3 text-center">
            ${isLow ? `<span class="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] bg-rose-950/60 text-rose-400 border border-rose-700/50">Low / Reorder</span>` : `<span class="inline-flex items-center px-1.5 py-0.5 rounded text-[10px] bg-emerald-950/40 text-emerald-400 border border-emerald-700/40">In-Tolerance</span>`}
          </td>
          ...
          <td class="py-2.5 px-3 text-right">
            <div class="flex items-center justify-end gap-1.5">
              ...
              ${isVerified ? `<button type="button" data-action="audit-verify" data-domain="${entry.domain}" data-id="${entry.id}" class="inline-flex items-center gap-1 px-2 py-1 rounded text-xs bg-emerald-950/60 text-emerald-300 border border-emerald-600/50 hover:bg-emerald-900/60">
                      <svg class="w-3.5 h-3.5" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><polyline points="20 6 9 17 4 12"/></svg>
                      <span>Counted (${entry.lastCounted})</span>
                    </button>` : `<button type="button" data-action="audit-verify" data-domain="${entry.domain}" data-id="${entry.id}" class="btn-metal-blue px-2.5 py-1 rounded text-xs font-bold shadow-xs">
                      Mark Verified
                    </button>`}
            </div>
          </td>
  ...
  // src/modules/qrCodeModule.ts
  var import_qrcode = __toESM(require_browser(), 1);
  var import_jsqr = __toESM(require_jsQR(), 1);
  function generateScanUrl(itemId, domain) {
  ...
  async function openQrModal(item, domain) {
    ...
    const linkInput = document.getElementById("qr-modal-url-input");
    ...
    if (testScanBtn) {
      testScanBtn.onclick = () => {
        ...
        openScanDetailsModal(item.id, window.__inventoryBundle, window.__onInventoryRefresh);
      };
    }
    modal.classList.remove("hidden");
  }
  function printAssetTag(item, domain, qrDataUrl) {
    let subHeader = "";
    let loc = "";
    let specs = "";
    let barcodeNum = "";
  ...
  function openScanDetailsModal(itemId, bundle, onUpdate) {
    let itemData = null;
    let domain = "material";
  ...
  function renderMobileScanModalContent(item, domain, bundle, onUpdate) {
    ...
    <div class="bg-[#121519] border border-[#282d36] rounded-lg p-3.5 space-y-2">
      ...
      <div class="text-xs text-[#8b98ab] font-mono">
        <span>SKU / ID: <strong class="text-[#cbd5e1]">${item.sku || item.partNumber || item.id}</strong></span>
      </div>
    </div>
  ...
  `;
    ...
    if (auditBtn) {
      auditBtn.onclick = () => {
        const stamp = (/* @__PURE__ */ new Date()).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" });
  ...
  function adjustItemQtyInScan(item, domain, delta, bundle, onUpdate) {
    let newQty = 0;
    if (domain === "material") {
      const s = item;
  ...
  function checkUrlForScanParam(bundle, onUpdate) {
    try {
  ...
  function openCameraScannerModal(bundle, onUpdate) {
    ...
    const manualInput = document.getElementById("camera-manual-id-input");
    const manualBtn = document.getElementById("camera-manual-id-btn");
    const fileInput = document.getElementById("camera-file-qr-input");
    let stream = null;
    let scanning = true;
    const stopCamera = () => {
      scanning = false;
  ...
  function parseScanResultToId(data) {
    try {
  ...
  // src/modules/codeTransfer.ts
  function generateInventoryCode(scope, stockItems2, toolItems2, machinePartItems2, selectedIds2) {
    let filteredStock = [];
    let filteredTools = [];
    let filteredParts = [];
    if (selectedIds2 && selectedIds2.size > 0) {
      filteredStock = stockItems2.filter((s) => selectedIds2.has(s.id));
      filteredTools = toolItems2.filter((t) => selectedIds2.has(t.id));
      filteredParts = machinePartItems2.filter((p) => selectedIds2.has(p.id));
    } else {
      if (scope === "all") {
        filteredStock = stockItems2;
        filteredTools = toolItems2;
        filteredParts = machinePartItems2;
      } else if (scope === "tools" || scope === "tools-with-codes") {
        filteredTools = scope === "tools-with-codes" ? toolItems2.filter((t) => t.sku && t.sku.trim().length > 0 || t.id && t.id.trim().length > 0) : toolItems2;
  ...
  function parseAndValidateCode(rawText) {
    ...
    try {
      ...
      if (parsed && typeof parsed === "object") {
        ...
        if (Array.isArray(parsed)) {
          parsed.forEach((item) => {
            if (!item) return;
            if (item.flutes !== void 0 || item.toolType !== void 0 || item.condition !== void 0) {
              tools.push(item);
              if (previewNames.length < 5) previewNames.push(item.name || item.sku);
            } else if (item.machineName !== void 0 || item.partNumber !== void 0 || item.criticality !== void 0) {
              parts.push(item);
              if (previewNames.length < 5) previewNames.push(item.name || item.partNumber);
            } else if (item.alloyGrade !== void 0 || item.shape !== void 0 || item.fullStockQty !== void 0) {
              stock.push(item);
              if (previewNames.length < 5) previewNames.push(item.name || item.sku);
        ...
        if (!Array.isArray(parsed) && !parsed.stock && !parsed.tools && !parsed.parts) {
          if (parsed.name || parsed.sku || parsed.partNumber) {
            if (parsed.flutes !== void 0 || parsed.toolType !== void 0) {
              tools.push(parsed);
            } else if (parsed.machineName !== void 0 || parsed.partNumber !== void 0) {
              parts.push(parsed);
            } else {
              stock.push(parsed);
            }
  ...
  function applyImportedInventory(mode, importedData, currentStock, currentTools, currentParts) {
    ...
    if (mode === "replace") {
      ...
      importedData.stock.forEach((item) => {
        ...
        if (idx >= 0) {
          updatedStock[idx] = { ...updatedStock[idx], ...item, lastUpdated: (/* @__PURE__ */ new Date()).toISOString().split("T")[0] };
          updatedCount++;
        } else {
          const newItem = {
            ...item,
            id: item.id || `stk-${Date.now()}-${Math.floor(Math.random() * 1e3)}`,
      ...
      importedData.tools.forEach((item) => {
        ...
        if (idx >= 0) {
          updatedTools[idx] = { ...updatedTools[idx], ...item, lastUpdated: (/* @__PURE__ */ new Date()).toISOString().split("T")[0] };
          updatedCount++;
        } else {
          const newTool = {
            ...item,
            id: item.id || `tool-${Date.now()}-${Math.floor(Math.random() * 1e3)}`,
            lastUpdated: (/* @__PURE__ */ new Date()).toISOString().split("T")[0]
          };
      ...
      importedData.parts.forEach((item) => {
        const idx = updatedParts.findIndex(
          (p) => p.id && item.id && p.id === item.id || p.partNumber && item.partNumber && p.partNumber.toLowerCase() === item.partNumber.toLowerCase()
        );
        if (idx >= 0) {
          updatedParts[idx] = { ...updatedParts[idx], ...item, lastUpdated: (/* @__PURE__ */ new Date()).toISOString().split("T")[0] };
          updatedCount++;
        } else {
          const newPart = {
            ...item,
            id: item.id || `part-${Date.now()}-${Math.floor(Math.random() * 1e3)}`,
            lastUpdated: (/* @__PURE__ */ new Date()).toISOString().split("T")[0]
          };
  ...
  var stockItems = [];
  var toolItems = [];
  var machinePartItems = [];
  var cutLogs = [];
  var activeTab = "materials";
  var selectedIds = /* @__PURE__ */ new Set();
  var activeOffcutItemId = null;
  var holdCleanupFunctions = [];
  var filterState = {
    searchQuery: "",
    category: "all",
    shape: "all",
  ...
  function updateCategoryChips() {
    ...
    if (!container) return;
    let chips = [{ id: "all", label: "All Categories" }];
    ...
    container.innerHTML = chips.map((c) => {
      ...
      return `
        <button 
          type="button" 
          data-category="${c.id}" 
          class="category-chip-btn px-2.5 py-1 rounded text-xs whitespace-nowrap transition-all ${isActive ? "bg-blue-600 text-white font-bold shadow-xs" : "metal-tab-btn text-[#8b98ab] hover:text-[#e2e8f0]"}"
        >${c.label}</button>
      `;
    }).join("");
  ...
  function updateLocationDropdown() {
    ...
    locSelect.innerHTML = `<option value="all">All Locations (${locs.size})</option>` + Array.from(locs).sort().map((l) => `<option value="${l}">${l}</option>`).join("");
  }
  function renderApp() {
  ...
  function attachHoldToTickToButtons() {
    document.querySelectorAll(".qty-hold-btn").forEach((btn) => {
  ...
  function adjustStockQty(id, delta, autoSave = true) {
    ...
    item.lastUpdated = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
  ...
  function adjustToolQty(id, delta, autoSave = true) {
    ...
    tool.lastUpdated = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
  ...
  function adjustPartQty(id, delta, autoSave = true) {
    const part = machinePartItems.find((p) => p.id === id);
    if (!part) return;
    part.qtyInStock = Math.max(0, part.qtyInStock + delta);
    part.lastUpdated = (/* @__PURE__ */ new Date()).toISOString().split("T")[0];
  ...
  function updateItemQtyDisplay(id, newQty, isLow) {
    const row = document.querySelector(`[data-item-id="${id}"]`);
    if (row) {
      const qtySpan = row.querySelector(".w-10, .w-8");
      if (qtySpan) {
        qtySpan.textContent = String(newQty);
        qtySpan.className = `w-10 text-center font-mono font-bold text-xs ${isLow ? "text-rose-400 font-extrabold" : "text-white"}`;
      }
    }
    ...
    if (auditRow) {
      ...
      if (qtySpan) {
        qtySpan.textContent = String(newQty);
        qtySpan.className = `w-12 text-center font-mono font-bold text-xs ${isLow ? "text-rose-400 font-extrabold" : "text-white"}`;
      }
    }
  }
  function filterMaterials() {
    return stockItems.filter((item) => {
      if (filterState.searchQuery) {
        ...
        const match = item.name.toLowerCase().includes(q) || item.sku.toLowerCase().includes(q) || item.alloyGrade.toLowerCase().includes(q) || item.rackLocation.toLowerCase().includes(q) || item.heatNumber && item.heatNumber.toLowerCase().includes(q) || item.allocatedJob && item.allocatedJob.toLowerCase().includes(q);
  ...
  function filterTools() {
    return toolItems.filter((tool) => {
      if (filterState.searchQuery) {
        const q = filterState.searchQuery;
        const match = tool.name.toLowerCase().includes(q) || tool.sku.toLowerCase().includes(q) || tool.toolType.toLowerCase().includes(q) || tool.sizeDiameter.toLowerCase().includes(q) || tool.location.toLowerCase().includes(q) || tool.assignedMachine && tool.assignedMachine.toLowerCase().includes(q);
        if (!match) return false;
      }
  ...
  function filterParts() {
    return machinePartItems.filter((part) => {
      if (filterState.searchQuery) {
        const q = filterState.searchQuery;
        const match = part.name.toLowerCase().includes(q) || part.partNumber.toLowerCase().includes(q) || part.machineName.toLowerCase().includes(q) || part.location.toLowerCase().includes(q) || part.oemBrand && part.oemBrand.toLowerCase().includes(q);
        if (!match) return false;
      }
...
` + exportPartsToCSV(machinePartItems);
      downloadFile(report, `shop_inventory_audit_${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}.csv`, "text/csv");
  ...
  function openBatchModal(actionType) {
    ...
    form.onsubmit = (e) => {
      e.preventDefault();
      if (actionType === "adjust-qty") {
        const val = parseInt(document.getElementById("batch-input-qty").value, 10);
        if (!isNaN(val)) {
          if (activeTab === "materials") {
        ...
        const loc = document.getElementById("batch-input-location").value.trim();
        if (loc) {
          if (activeTab === "materials") {
        ...
        const stat = document.getElementById("batch-input-status").value;
        if (activeTab === "materials") {
          stockItems.forEach((s) => selectedIds.has(s.id) && (s.status = stat));
          saveStock(stockItems);
        }
      }
      closeModal();
  ...
  function setupModalForms() {
    ...
    const matForm = document.getElementById("form-material");
    const matShapeSelect = document.getElementById("input-shape");
    ...
    document.querySelectorAll("#alloy-presets-container .preset-btn").forEach((btn) => {
      btn.addEventListener("click", () => {
        ...
        if (preset) document.getElementById("input-alloy").value = preset;
        if (cat) document.getElementById("input-category").value = cat;
        if (name) document.getElementById("input-name").value = name;
      });
    });
    matForm?.addEventListener("submit", (e) => {
      e.preventDefault();
      const id = document.getElementById("input-material-id").value;
      const name = document.getElementById("input-name").value.trim();
      const category = document.getElementById("input-category").value;
      const alloyGrade = document.getElementById("input-alloy").value.trim();
      ...
      const diameter = parseFloat(document.getElementById("input-diameter").value) || void 0;
      const width = parseFloat(document.getElementById("input-width").value) || void 0;
      const thickness = parseFloat(document.getElementById("input-thickness").value) || void 0;
      const wallThickness = parseFloat(document.getElementById("input-wall").value) || void 0;
      const length = parseFloat(document.getElementById("input-length").value) || 72;
      const fullStockQty = parseInt(document.getElementById("input-qty").value, 10) || 0;
      const minThreshold = parseInt(document.getElementById("input-min-threshold").value, 10) || 2;
      const costPerUnit = parseFloat(document.getElementById("input-cost").value) || 0;
      const rackLocation = document.getElementById("input-location").value.trim() || "Rack General";
      const status = document.getElementById("input-status").value;
      const heatNumber = document.getElementById("input-heat").value.trim() || void 0;
      const supplier = document.getElementById("input-supplier").value.trim() || void 0;
      const allocatedJob = document.getElementById("input-job").value.trim() || void 0;
      const notes = document.getElementById("input-notes").value.trim() || void 0;
      if (id) {
    ...
    toolForm?.addEventListener("submit", (e) => {
      e.preventDefault();
      const id = document.getElementById("input-tool-id").value;
      const name = document.getElementById("input-tool-name").value.trim();
      const category = document.getElementById("input-tool-category").value;
      const sku = document.getElementById("input-tool-sku").value.trim();
      const sizeDiameter = document.getElementById("input-tool-size").value.trim();
      const flutes = document.getElementById("input-tool-flutes").value.trim();
      const coating = document.getElementById("input-tool-coating").value.trim();
      const shankOrHolder = document.getElementById("input-tool-shank").value.trim();
      const insertGrade = document.getElementById("input-tool-insert-grade").value.trim();
      const location = document.getElementById("input-tool-location").value.trim();
      const qtyInStock = parseInt(document.getElementById("input-tool-qty").value, 10) || 0;
      const minThreshold = parseInt(document.getElementById("input-tool-min").value, 10) || 2;
      const costPerUnit = parseFloat(document.getElementById("input-tool-cost").value) || 0;
      const condition = document.getElementById("input-tool-condition").value;
      const assignedMachine = document.getElementById("input-tool-machine").value.trim();
      const supplier = document.getElementById("input-tool-supplier").value.trim();
      const notes = document.getElementById("input-tool-notes").value.trim();
      if (id) {
        ...
        if (existing) {
          Object.assign(existing, {
            ...
            toolType: document.getElementById("input-tool-category").selectedOptions[0].text,
            sizeDiameter,
            flutes,
            coating,
            shankOrHolder,
    ...
    const partForm = document.getElementById("form-part");
    ...
    partForm?.addEventListener("submit", (e) => {
      e.preventDefault();
      const id = document.getElementById("input-part-id").value;
      const name = document.getElementById("input-part-name").value.trim();
      const machineName = document.getElementById("input-part-machine").value.trim();
      const partNumber = document.getElementById("input-part-number").value.trim();
      const category = document.getElementById("input-part-category").value;
      const criticality = document.getElementById("input-part-criticality").value;
      const location = document.getElementById("input-part-location").value.trim();
      const qtyInStock = parseInt(document.getElementById("input-part-qty").value, 10) || 0;
      const minThreshold = parseInt(document.getElementById("input-part-min").value, 10) || 1;
      const costPerUnit = parseFloat(document.getElementById("input-part-cost").value) || 0;
      const oemBrand = document.getElementById("input-part-brand").value.trim();
      const supplier = document.getElementById("input-part-supplier").value.trim();
      const serviceIntervalNotes = document.getElementById("input-part-notes").value.trim();
      if (id) {
    ...
    const cutForm = document.getElementById("form-cut");
    ...
    cutForm?.addEventListener("submit", (e) => {
      e.preventDefault();
      const stockId = document.getElementById("cut-stock-select").value;
      const cutLength = parseFloat(document.getElementById("cut-length").value);
      const cutQty = parseInt(document.getElementById("cut-qty").value, 10) || 1;
      const kerf = parseFloat(document.getElementById("cut-kerf").value) || 0.125;
      const jobId = document.getElementById("cut-job-id").value.trim() || "General Job";
      const machinist = document.getElementById("cut-machinist").value.trim() || "Machinist";
      const autoSaveOffcut = document.getElementById("cut-save-offcut").checked;
      ...
      if (item.fullStockQty > 0) {
        ...
        if (autoSaveOffcut && remnantLength >= 4) {
          createdDrop = true;
          const newOffcut = {
            id: "oc-" + Date.now(),
      ...
      addCutLog({
        ...
        offcutLength: createdDrop ? Number(remnantLength.toFixed(2)) : void 0
      });
      saveStock(stockItems);
  ...
  function updateDynamicDimensionsUI(shape) {
  ...
  function openMaterialModal(id) {
    ...
    if (id) {
      ...
      document.getElementById("input-material-id").value = item.id;
      document.getElementById("input-name").value = item.name;
      document.getElementById("input-category").value = item.category;
      document.getElementById("input-alloy").value = item.alloyGrade;
      document.getElementById("input-shape").value = item.shape;
      document.getElementById("input-sku").value = item.sku || "";
      document.getElementById("input-unit").value = item.unit || "in";
      document.getElementById("input-diameter").value = item.diameter ? String(item.diameter) : "";
      document.getElementById("input-width").value = item.width ? String(item.width) : "";
      document.getElementById("input-thickness").value = item.thickness ? String(item.thickness) : "";
      document.getElementById("input-wall").value = item.wallThickness ? String(item.wallThickness) : "";
      document.getElementById("input-length").value = String(item.length || 72);
      document.getElementById("input-qty").value = String(item.fullStockQty);
      document.getElementById("input-min-threshold").value = String(item.minThreshold);
      document.getElementById("input-cost").value = String(item.costPerUnit || 0);
      document.getElementById("input-location").value = item.rackLocation;
      document.getElementById("input-status").value = item.status;
      document.getElementById("input-heat").value = item.heatNumber || "";
      document.getElementById("input-supplier").value = item.supplier || "";
      document.getElementById("input-job").value = item.allocatedJob || "";
      ...
      document.getElementById("input-length").value = "72";
      document.getElementById("input-qty").value = "1";
      document.getElementById("input-min-threshold").value = "2";
  ...
  function openToolModal(id) {
    ...
    if (id) {
      ...
      if (deleteBtn) deleteBtn.classList.remove("hidden");
      document.getElementById("input-tool-id").value = tool.id;
      document.getElementById("input-tool-name").value = tool.name;
      document.getElementById("input-tool-category").value = tool.category;
      document.getElementById("input-tool-sku").value = tool.sku || "";
      document.getElementById("input-tool-size").value = tool.sizeDiameter;
      document.getElementById("input-tool-flutes").value = tool.flutes || "";
      document.getElementById("input-tool-coating").value = tool.coating || "";
      document.getElementById("input-tool-shank").value = tool.shankOrHolder || "";
      document.getElementById("input-tool-insert-grade").value = tool.insertGrade || "";
      document.getElementById("input-tool-location").value = tool.location;
      document.getElementById("input-tool-qty").value = String(tool.qtyInStock);
      document.getElementById("input-tool-min").value = String(tool.minThreshold);
      document.getElementById("input-tool-cost").value = String(tool.costPerUnit || 0);
      document.getElementById("input-tool-condition").value = tool.condition;
      document.getElementById("input-tool-machine").value = tool.assignedMachine || "";
      document.getElementById("input-tool-supplier").value = tool.supplier || "";
      document.getElementById("input-tool-notes").value = tool.notes || "";
    } else {
      if (title) title.textContent = "+ Add Tooling / Cutter";
      if (deleteBtn) deleteBtn.classList.add("hidden");
      document.getElementById("input-tool-id").value = "";
      document.getElementById("input-tool-qty").value = "5";
      document.getElementById("input-tool-min").value = "2";
    }
    modal.classList.remove("hidden");
  }
  function openPartModal(id) {
    ...
    if (id) {
      ...
      if (deleteBtn) deleteBtn.classList.remove("hidden");
      document.getElementById("input-part-id").value = part.id;
      document.getElementById("input-part-name").value = part.name;
      document.getElementById("input-part-machine").value = part.machineName;
      document.getElementById("input-part-number").value = part.partNumber;
      document.getElementById("input-part-category").value = part.category;
      document.getElementById("input-part-criticality").value = part.criticality;
      document.getElementById("input-part-location").value = part.location;
      document.getElementById("input-part-qty").value = String(part.qtyInStock);
      document.getElementById("input-part-min").value = String(part.minThreshold);
      document.getElementById("input-part-cost").value = String(part.costPerUnit || 0);
      document.getElementById("input-part-brand").value = part.oemBrand || "";
      document.getElementById("input-part-supplier").value = part.supplier || "";
      document.getElementById("input-part-notes").value = part.serviceIntervalNotes || "";
    } else {
      if (title) title.textContent = "+ Add Machine Spare Component";
      if (deleteBtn) deleteBtn.classList.add("hidden");
      document.getElementById("input-part-id").value = "";
      document.getElementById("input-part-qty").value = "2";
      document.getElementById("input-part-min").value = "1";
    }
    modal.classList.remove("hidden");
  }
  function openCutModal(stockId) {
    const modal = document.getElementById("modal-cut");
    const select = document.getElementById("cut-stock-select");
    if (!modal || !select) return;
    select.innerHTML = stockItems.map(
      (s) => `<option value="${s.id}" ${s.id === stockId ? "selected" : ""}>${s.name} (${s.fullStockQty} bars @ ${s.length}") - ${s.rackLocation}</option>`
    ).join("");
    modal.classList.remove("hidden");
  }
  function openOffcutsModal(itemId) {
    ...
    const form = document.getElementById("form-add-offcut");
    ...
    form.onsubmit = (e) => {
      e.preventDefault();
      const len = parseFloat(document.getElementById("new-offcut-length").value);
      const qty = parseInt(document.getElementById("new-offcut-qty").value, 10) || 1;
      const loc = document.getElementById("new-offcut-loc").value.trim() || `Drop Bin ${item.rackLocation}`;
      if (len > 0) {
        item.offcuts = item.offcuts || [];
        item.offcuts.push({
          id: "oc-" + Date.now(),
  ...
  function renderOffcutsList(item) {
    ...
    container.innerHTML = offcuts.map(
      (oc) => `
      ...
      tbody.innerHTML = cutLogs.map(
        ...
        <tr class="hover:bg-[#181c23]">
          ...
          <td class="py-2 px-2.5">
            ${log.createdOffcut ? `<span class="text-emerald-400 font-mono">${log.offcutLength}" saved</span>` : `<span class="text-[#5a6578]">No Drop</span>`}
          </td>
        </tr>
      `
      ).join("");
  ...
  function setupToolsMenu() {
    ...
    document.getElementById("btn-export-csv")?.addEventListener("click", () => {
      if (activeTab === "materials") {
        downloadFile(exportStockToCSV(stockItems), `machine_shop_materials_${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}.csv`, "text/csv");
      } else if (activeTab === "tools") {
        downloadFile(exportToolsToCSV(toolItems), `machine_shop_tools_${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}.csv`, "text/csv");
      } else if (activeTab === "machine-parts") {
        downloadFile(exportPartsToCSV(machinePartItems), `machine_shop_parts_${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}.csv`, "text/csv");
      } else {
      ...
      if (cols.length >= 4) {
        if (activeTab === "tools") {
          ...
          machinePartItems.unshift({
            ...
            category: cols[3] || "belts-pulleys",
            criticality: cols[4] || "standard",
          ...
          stockItems.unshift({
            ...
            category: cols[2] || "aluminum",
            alloyGrade: cols[3] || "6061-T6",
            shape: cols[4] || "round-bar",
            length: 72,
            unit: "in",
  ...
  var activeCodeTransferScope = "tools-with-codes";
  var activeSingleExportToolId = null;
  function openCodeTransferModal(initialTab = "export", scope = "tools-with-codes", singleToolId) {
    ...
    const scopeSelect = document.getElementById("code-export-scope");
    if (scopeSelect) {
  ...
  function switchCodeTransferTab(tab) {
  ...
  function refreshExportCodeView() {
    const textarea = document.getElementById("code-export-textarea");
    const statsBadge = document.getElementById("code-export-stats-text");
    if (!textarea) return;
    let selectedSet;
    if (activeSingleExportToolId) {
      selectedSet = /* @__PURE__ */ new Set([activeSingleExportToolId]);
    ...
    if (statsBadge) {
      ...
      if (activeCodeTransferScope === "tools-with-codes" || activeCodeTransferScope === "tools") {
        const toolCount = selectedSet ? toolItems.filter((t) => selectedSet.has(t.id)).length : toolItems.filter((t) => activeCodeTransferScope === "tools" || t.sku && t.sku.trim().length > 0).length;
        ...
        const total = (selectedSet ? stockItems.filter((s) => selectedSet.has(s.id)).length : stockItems.length) + (selectedSet ? toolItems.filter((t) => selectedSet.has(t.id)).length : toolItems.length) + (selectedSet ? machinePartItems.filter((p) => selectedSet.has(p.id)).length : machinePartItems.length);
  ...
  function validateAndInspectImport() {
    const textarea = document.getElementById("code-import-textarea");
    ...
    if (result.isValid) {
      ...
      previewPills.innerHTML = sampleItems.map(
        (s) => `
  ...
  function setupCodeTransferSystem() {
    ...
    scopeSelect?.addEventListener("change", () => {
      activeCodeTransferScope = scopeSelect.value;
      activeSingleExportToolId = null;
      refreshExportCodeView();
    });
    ...
    const exportTextarea = document.getElementById("code-export-textarea");
    ...
    const importTextarea = document.getElementById("code-import-textarea");
    importTextarea?.addEventListener("input", () => {
      validateAndInspectImport();
    });
    const fileInput = document.getElementById("code-import-file");
    ...
    doImportBtn?.addEventListener("click", async () => {
      ...
      const replaceRadio = document.querySelector('input[name="code-import-strategy"][value="replace"]');
      ...
      showToast(
        strategy === "replace" ? `Replaced inventory: Loaded ${totalAffected} items from code` : `Synced code: Added ${res.stats.added} new, updated ${res.stats.updated} existing items`,
        "success"
      );
      renderApp();
    });
